...
\end{align}
So we can determine such a state by $M \times N$ numbers $c_{mn}$ here.  If we can write $\ket{\psi}$ as a product of the individual staein \eqref{eq:psientangled}, we call the state \emph{entangled} . Most states are entangled states. Equation \eqref{eq:psientangled} thus only describes a small subset of all possible states.
