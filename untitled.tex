...
In quantum mechanics, we follow a similar approach of first identifying the possible outcomes. But instead of describing the outcomes with real numbers, we now associate a complex number $\alpha_i$ to each outcome $(\alpha_1, \cdots, p\alpha_N)$, with $\alpha_i \in \mathbb{C}$. Given that they should also describe some probability 
