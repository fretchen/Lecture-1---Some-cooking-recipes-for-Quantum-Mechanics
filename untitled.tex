...
\begin{align}
				\langle{\psi_1} | \psi_2}=(\braket{\psi_2 | \psi_1})^*.
			
\end{align}
\paragraph{Example 1: Stern-Gerlach experiment \index{Stern-Gerlach!experiment}.}

Here, we are only interested in the internal degree of freedom of the \ch{Ag} atoms:
...
\end{itemize}
\marginpar{\scriptsize{Lecture 2}}
...
\paragraph{Schrödinger Equation.} The Schrödinger equation \index{Schrödinger equation} reads

\begin{align}
						i\hbar\diffp{}{t}\ket{\psi(t)}=\hat{H}(t)\ket{\psi(t)}.
					
\end{align}
										In general, the Hamilton operator \index{Hamilton operator} $\hat{H}$ is time-dependent. For a time-independent Hamilton operator $\hat{H}$, we can find eigenstates $\ket{\phi_n}$ with corresponding eigenenergies $E_n$ \index{eigenenergy}:
